        ...
        [#list dimensions as d]
             ...
             [#list filters as f]
                    ...
                    [#elseif f.dimension.id == d.id]
                        <dt>[@label d /]</dt>
                        <dd><span class="label label-default filter" data-ref="${d.id}" data-index="${idx}">[@label f /] [#if RequestParameters.sc?? && f.code??](${f.code})[/#if]<span class="fas fa-times"></span></span></dd>
                        [#assign idx = idx + 1 /]
                    [/#if]
            [/#list]
        [/#list]
